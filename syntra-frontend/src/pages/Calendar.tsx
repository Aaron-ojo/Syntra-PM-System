  ...
  const loadAllTasks = async () => {
    ...
    try {
      ...
      if (selectedProjectId) {
        allTasks = await taskService.getTasks(selectedProjectId);
      } else {
              ...
              <div className="grid grid-cols-7 gap-1">
                {days.map((date, index) => {
                  ...
                  const hasOverdueTasks = tasksForDate.some(
                    (t) =>
                      isOverdue(new Date(t.due_date!)) && t.status !== "done",
                  );
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <span
                          className={`text-sm font-medium ${isCurrentDate ? "text-blue-600 dark:text-blue-400" : "text-gray-700 dark:text-gray-300"} ${hasOverdueTasks ? "text-red-500 dark:text-red-400" : ""}`}
                        >
                          {date.getDate()}
                        </span>
              ...
              <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800">
                <div>
                  ...
                  {isOverdue(selectedDate) && (
                    <p className="text-xs text-red-500 mt-0.5">Overdue date</p>
                  )}
                </div>
                <button
                  onClick={() => setSelectedDate(null)}
                  ...
                  getTasksForDate(selectedDate).map((task) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <CheckSquare className="w-5 h-5 text-gray-400 flex-shrink-0 ml-3" />
                      </div>
                    </div>
                  ))
                )}
